...
import art.tessell.editor.models.{EditorConfig, EditorState, Tool, ViewTransform}
import art.tessell.editor.operations.{AddCopyOperations, ProximityQuery, ViewOperations}
  ...
  def handleMouseMove(event: MouseEvent): Unit =
    ...
    if !AddCopyOperations.updateActiveDrag(event.clientX, event.clientY) then
      ...
      if dragging then
        ...
        // Every Add Copy tool reveals only the anchors near the cursor (ADR-013 amendment).
        else if
          tool == Tool.TranslateCopy || tool == Tool.RotateCopy ||
          tool == Tool.ReflectCopy || tool == Tool.GlideReflectCopy
        then
          AddCopyOperations.updateAddCopyHover(event.clientX, event.clientY)

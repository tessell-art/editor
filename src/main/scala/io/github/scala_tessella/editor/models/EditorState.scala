...
import io.github.scala_tessella.editor.i18n.Locale
import io.github.scala_tessella.editor.utils.{LocaleStorage, RecentFilesStorage}
...
object EditorState:
  ...
  /** Most-recent-first list of recently-opened files. Initialized from localStorage at startup; mutated
    * through [[io.github.scala_tessella.editor.operations.RecentFilesOperations]] which keeps storage in
    * sync.
    */
  ...
  /** Currently-selected UI locale. Initialized from localStorage at startup; the top-bar language toggle
    * writes back through [[io.github.scala_tessella.editor.utils.LocaleStorage]] to keep the preference
    * across reloads.
    */

...
object TouchEventHandler:
  ...
  private def screenToWorld(screenX: Double, screenY: Double, transform: ViewTransform): (Double, Double) =
    val (panX, panY, scale, rotationDegrees) = (transform.panX, transform.panY, transform.scale, transform.rotationDegrees)
  ...
  private def worldToScreenNoPan(worldX: Double, worldY: Double, scale: Double, rotationDegrees: Double): (Double, Double) =
    val rotRad = Math.toRadians(rotationDegrees)
    val rotationCenterX = EditorConfig.canvasCenterX
    val rotationCenterY = EditorConfig.canvasCenterY

...
class GeometrySpec extends FunSuite:
  ...
  test("normalizeDeltaAngle should handle angle wrapping") {
    ...
    val delta  = normalizeDeltaAngle(angle2, angle1)
  ...
  test("transformPointsForSvg should apply transformations correctly") {
    ...
    val result = transformPointsForSvg(points, scale, offsetX, offsetY)

    assertEquals(result(0)._1, 12.0, epsilon)
    assertEquals(result(0)._2, 22.0, epsilon)
    assertEquals(result(1)._1, 14.0, epsilon)
    assertEquals(result(1)._2, 24.0, epsilon)
  }
  ...
  test("fitPointsToViewBox should calculate correct dimensions") {
    ...
    assertEquals(offsetX, 2.0, epsilon)
    assertEquals(offsetY, 2.0, epsilon)
  }
  ...
  test("Bounds.fromPoints should calculate correct bounds") {
    ...
    assertEquals(bounds.minX, 1.0, epsilon)
    assertEquals(bounds.maxX, 5.0, epsilon)
    assertEquals(bounds.minY, 1.0, epsilon)
    assertEquals(bounds.maxY, 6.0, epsilon)
  ...
  test("Radian.normalizeDelta should handle various angles") {
    ...
    assertEquals(Radian.normalizeDelta(Radian(0)).toDouble, 0.0, epsilon)
    assertEquals(Radian.normalizeDelta(Radian(Pi)).toDouble, Pi, epsilon)
    ...
    val largePosAngle = Radian.normalizeDelta(Radian(3 * Pi))
    ...
    val largeNegAngle = Radian.normalizeDelta(Radian(-3 * Pi))

...
class EraserProximityQuerySpec extends FunSuite with EditorStateFixture:
  ...
  test("allClickablePoints returns 9 points for a square (4 vertices + 4 midpoints + 1 center)") {
    ...
    val points = EraserProximityQuery.allClickablePoints(tiling)
  ...
  test("allClickablePoints returns empty list for empty tiling") {
    val points = EraserProximityQuery.allClickablePoints(TilingDCEL.empty)
  ...
  test("nearbyPoints returns empty when pointer is far from all points") {
    val tiling    = TilingBuilders.freshSquare()
    val allPoints = EraserProximityQuery.allClickablePoints(tiling)
    val transform = ViewTransform()
    // Very far away in SVG space
    val farPoint  = Point(-9999.0, -9999.0)
    val nearby    = EraserProximityQuery.nearbyPoints(farPoint, allPoints, transform, 30.0)
  ...
  test("nearbyPoints returns points within radius") {
    ...
    val nearby     = EraserProximityQuery.nearbyPoints(nearPos, allPoints, transform, 30.0)
  ...
  test("nearbyPoints respects radius — tight radius excludes distant points") {
    ...
    val tightNearby = EraserProximityQuery.nearbyPoints(offsetPos, allPoints, transform, 10.0)
    val wideNearby  = EraserProximityQuery.nearbyPoints(offsetPos, allPoints, transform, 25.0)

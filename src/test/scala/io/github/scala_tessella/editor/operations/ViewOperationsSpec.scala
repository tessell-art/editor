...
class ViewOperationsSpec extends FunSuite:
  ...
  test("transformCoordinates should rotate points correctly") {
    ...
    assert(Math.abs(rotated.head.x - 0.0) < delta)
    assert(Math.abs(rotated.head.y - 1.0) < delta)
    assert(Math.abs(rotated(1).x - -1.0) < delta)
    assert(Math.abs(rotated(1).y - 0.0) < delta)
  }
  ...
  test("calculateTilingCenter should return the center of the bounds") {
    ...
    assertEquals(center.x, 50.0, delta)
    assertEquals(center.y, 50.0, delta)
  }
  ...
  test("createUpdatedViewTransform should update scale and pan") {
    ...
    assertEquals(updatedTransform.pan.x, 100.0, delta)
    assertEquals(updatedTransform.pan.y, -100.0, delta)
  ...
  test("inverseTransform should correctly calculate world coordinates for the view center") {
    ...
    assertEquals(world.x, 275.0, delta)
    assertEquals(world.y, 525.0, delta)
  }
  ...
  test("forwardTransform should correctly calculate screen coordinates") {
    ...
    assertEquals(screen.x, 350.0, delta)
    assertEquals(screen.y, 350.0, delta)
  }
  ...
  test("calculateRotatedPan should compute pan to keep a point stationary during rotation") {
    ...
    assertEquals(pan.x, 50.0, delta)
    assertEquals(pan.y, -50.0, delta)
  }
  ...
  test("should return a new transform when fitting a tiling to the canvas") {
    ...
    assertEquals(transform.pan.x, -2820.0, delta)
    assertEquals(transform.pan.y, -2115.0, delta)
  ...
  test("clampPanToBounds should leave pan unchanged when tiling bounds are inside the canvas") {
    val bounds = Bounds(Point(100, 100), Point(700, 500))
    val pan    = Point(0, 0)
    // Tiling spans [100,700] × [100,500]. Pan range to keep visible:
    //   x ∈ [-700, 800-100=700], y ∈ [-500, 600-100=500]. Pan(0,0) is well within.
    val out    = ViewOperations.clampPanToBounds(pan, viewW = 800, viewH = 600, bounds)
    assertEquals(out, pan)
  }

  test("clampPanToBounds should clamp pan when it would push the tiling fully off-screen to the right") {
    val bounds = Bounds(Point(100, 100), Point(700, 500))
    val pan    = Point(2000, 0) // way off-screen to the right
    // maxPanX = viewW - bounds.min.x = 800 - 100 = 700
    val out    = ViewOperations.clampPanToBounds(pan, viewW = 800, viewH = 600, bounds)
    assertEquals(out.x, 700.0, delta)
    assertEquals(out.y, 0.0, delta)
  }

  test("clampPanToBounds should clamp pan when it would push the tiling fully off-screen to the left") {
    val bounds = Bounds(Point(100, 100), Point(700, 500))
    val pan    = Point(-2000, 0)
    // minPanX = -bounds.max.x = -700
    val out    = ViewOperations.clampPanToBounds(pan, viewW = 800, viewH = 600, bounds)
    assertEquals(out.x, -700.0, delta)
    assertEquals(out.y, 0.0, delta)
  }

  test("clampPanToBounds should clamp pan independently on x and y") {
    val bounds = Bounds(Point(100, 100), Point(700, 500))
    val pan    = Point(2000, -2000)
    val out    = ViewOperations.clampPanToBounds(pan, viewW = 800, viewH = 600, bounds)
    // maxPanX = 800 - 100 = 700, minPanY = -500
    assertEquals(out.x, 700.0, delta)
    assertEquals(out.y, -500.0, delta)
  }

  test("screenBoundsNoPan returns None for empty coords") {
    val out =
      ViewOperations.screenBoundsNoPan(Nil, ViewTransform(scale = 1.0), EditorConfig.canvasCenter)
    assertEquals(out, None)
  }

  test("screenBoundsNoPan with identity transform reflects scaled coordinates around viewCenter") {
    // forwardTransform with scale=1, rotation=0, viewCenter=center: returns world * 1 = world.
    val coords = List(Point(0, 0), Point(100, 50))
    val out    =
      ViewOperations.screenBoundsNoPan(coords, ViewTransform(scale = 1.0), EditorConfig.canvasCenter)
    assert(out.isDefined)
    val b      = out.get
    assertEquals(b.min, Point(0, 0))
    assertEquals(b.max, Point(100, 50))
  }

  test("screenBoundsNoPan with scale=2 doubles the screen-space extent") {
    val coords = List(Point(0, 0), Point(100, 50))
    val out    =
      ViewOperations.screenBoundsNoPan(coords, ViewTransform(scale = 2.0), EditorConfig.canvasCenter)
    assert(out.isDefined)
    val b      = out.get
    assertEqualsDouble(b.width, 200.0)
    assertEqualsDouble(b.height, 100.0)
  }

  private def assertEqualsDouble(obtained: Double, expected: Double): Unit =
    assert(
  ...
  test("should correctly calculate transform with rotation") {
    ...
    assertEquals(transform.pan.x, -1375.0, delta)
    assertEquals(transform.pan.y, -1500.0, delta)

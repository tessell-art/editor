...
import io.github.scala_tessella.editor.models.MenuShortcuts.{MenuAction, Shortcut}
...
class MenuShortcutsSpec extends FunSuite:
  ...
  test("labelOf preserves the exact strings that MenuBarComponent used to hard-code") {
    ...
    assertEquals(MenuShortcuts.labelOf(MenuAction.EditDoubleToInfinite), "D")
    ...
    assertEquals(MenuShortcuts.labelOf(MenuAction.ViewRotateLeft), "E")
    assertEquals(MenuShortcuts.labelOf(MenuAction.ViewRotateRight), "R")
  }
